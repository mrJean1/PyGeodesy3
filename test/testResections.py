...
class Tests(TestsBase):
    ...
    def testResections(self, C_, V3d):
        ...
        self.test(collins5.__name__, fstr(t[2:], prec=4), r)
#       self.test(collins.__name__, C_(A).collins(C, B, 109.5125, 115.0889), t, nl=1)  # BACKWARD
...
#       self.test(tienstra.__name__, C_(A).tienstra(B, C, 115.0889, None, 109.5125), t, nl=1)  # BACKWARD
...
#       self.test(collins.__name__, collins(A, C, B, 109.3, 115.1), t, nl=1)  # BACKWARD
...
#       self.test(tienstra.__name__, tienstra(A, B, C, 115.1, beta=135.6, gamma=109.3), t, nl=1)  # BACKWARD

    def testSnellius(self):
        n = snellius3.__name__

        c = triSide(10, 30, PI_4)
        rA, rB, rC, _, _ = triAngle5(10, 30, c)  # BACKWARD triAngle4
        t = triSide4(rA, rB, c)
    ...
    def testTri(self):  # for coverage
        ...
        t = triAngle5(1, 2, EPS0 / 2)  # BACKWARD triAngle4
