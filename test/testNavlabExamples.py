...
# Example 1: A and B to delta
    ...
    d = a.deltaTo(b)
    t.test(1, 'delta', d, '[331730.863, 332998.501, 17398.304]')
    t.test(1, 'delta', d.toRepr(prec=3), '[N:331730.863, E:332998.501, D:17398.304]')  # BACKWARD [L:470357.384, B:45.109°, E:-2.12°]
    t.test(1, 'elevation', d.elevation, -2.1198, fmt='%.4f')
    t.test(1, 'bearing', d.bearing, 45.109, fmt='%.3f')  # BACKWARD  azimuth
    t.test(1, 'length', d.length, 470357.384, fmt='%.3f')  # BACKWARD slantrange

    d = a.deltaTo(b, Ned=Ned)  # kwarg Ned, new in pygeodesy3 22.09.17+
    t.test(1, 'delta', d, '[331730.863, 332998.501, 17398.304]', nl=1)
    t.test(1, 'delta', d.__class__, Ned)
    d = a.deltaTo(b, Ned=Ned4Tuple)
    t.test(1, 'delta', d, '(331730.863099, 332998.501491, 17398.304211, Ltp(lat0=1.0, lon0=2.0, height0=3.0, ', known=startswith)
    t.test(1, 'delta', d.__class__, Ned4Tuple)
    t.test(1, 'delta', d.ltp.ecef.__class__, EcefVeness)
...
#   t.test(2, 'Nvector', n.toStr(prec=3), '[1.0, 2.0, 3.0, +400.00]')
    ...
    d = ellipsoidalNvector.Ned(3000, 2000, 100)
    t.test(2, 'delta', d, '[3000.0, 2000.0, 100.0]')  # ++
    t.test(2, 'delta', d.toRepr(prec=3), '[N:3000.0, E:2000.0, D:100.0]')  # BACKWARD [L:3606.938, B:33.69°, E:-1.589°]
    c = b.destinationNed(d)  # JSname: destinationPoint
    ...
    d = a.deltaTo(b)  # ++
    t.test(2, 'delta', d.toStr(prec=0), '[-86126, -78900, 1069]')  # ++
    t.test(2, 'delta', d.toRepr(prec=3), '[N:-86125.881, E:-78900.088, D:1069.198]')  # BACKWARD [L:116807.681, B:222.493°, E:-0.524°]
    c = a.destinationNed(d)  # JSname: destinationPoint, c.height = -9.3e-10
...
# <https://GitHub.com/ChrisVeness/geodesy/blob/master/latlon-nvector-ellipsoidal.js>
    d = ellipsoidalNvector.toNed(116809.178, 222.493, -0.5416)  # BACKWARD
    TestsBase.test(t, 'toNed', d.toStr(prec=1), '[-78901.1, -86126.6, 1104.1]', nl=1)  # [N:-86126.6, E:-78901.1, D:1104.1]'
    TestsBase.test(t, 'bearing',   d.bearing, '227.507',  fmt='%.3f')  # BACKWARD azimuth '222.493'
    TestsBase.test(t, 'elevation', d.elevation, '-0.5416', fmt='%.4f')
    TestsBase.test(t, 'length',    d.length, '116809.178',  fmt='%.3f')  # BACKWARD slantrange
    TestsBase.test(t, 'xyz', d.xyz.toStr(prec=1), '(-86126.6, -78901.1, -1104.1)')  # BACKWARD d.toVector3d 1104.1
