...
from bases import GeodSolve, geographiclib, isPython37, \
...
class Tests(TestsBase):
    ...
    def test3(self, LatLon, K=True, X=False):
        ...
        if LatLon is LatLon_:
            ...
            from pygeodesy3.polygonal.points import _Array2LatLon
...
#       self.test('to2ab ', p.to2ab(), p.philam)
        ...
        self.test('toNvector', p.toNvector().toStr(prec=5), '(0.01386, -0.39691, -0.91775', known=startswith)  # BACKWARD , 0)
        self.test('toNvector', p.toNvector(Nvector=None),   '(0.01386, -0.396906, -0.917755, 0)')
...
if __name__ == '__main__':  # PYCHOK internal error?
    ...
    t.test3(ellipsoidalKarney.LatLon, K=geographiclib)
