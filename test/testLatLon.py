...
class Tests(TestsBase):
    ...
    def testLatLon(self, module, Sph=False, Nv=False, X=False, GS=False):  # MCCABE 108
        ...
        if hasattr(LatLon, 'initialBearingTo'):
            ...
            # courtesy of U{Inmars Didaitis (bakakaldsas)<https://GitHub.com/mrJean1/PyGeodesy3/issues/56>}
        ...
        if hasattr(LatLon, 'isenclosedBy'):
            # courtesy of U{Maranov<https://GitHub.com/mrJean1/PyGeodesy3/issues/53>}
...
#       self.testReturnType(p.to3xyz(),             Vector3Tuple, 'to3xyz')
        ...
        if not Nv:
            ...
            self.test('toNvector', p.toNvector(), '(0.61566, 0.0, 0.78801)' if Sph  # BACKWARD  not 0)
                                             else '(0.615661, 0.0, 0.788011, 0)', known=startswith)  # BACKWARD  not 0)
        self.test('toVector',   p.toVector(), '(3922384.659522, 0.0, 5020423.423198)' if Sph
                                         else '(3934960.466675, 0.0, 5002803.345483)')  # BACKWARD (0.615661, 0.0, 0.788011)
        self.test('toVector3d', p.toVector3d(), '(0.61566, 0.0, 0.78801)' if Sph
                                           else '(0.61823, 0.0, 0.786)')  # BACKWARD (0.61566, 0.0, 0.78801)
        ...
        try:  # courtesy of theirix <https://GitHub.com/mrJean1/PyGeodesy3/issues/71>
            a = module.areaOf
