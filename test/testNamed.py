...
class Tests(TestsBase):
    ...
    def testNamed_classes(self, _Nbase, _Nclass, _attr_, _Ndict):
        for c in self.pygeodesy3_classes(Base=_Nbase, deprecated=True):
            n = c.__name__
...
#           self.test(m, t, _Ndict.get(n, 'signature'), known=n in ('ClipCS3Tuple', 'Ned3Tuple'))
    ...
    def testUnregister(self):
        ...
        from pygeodesy3 import Conics, Ellipsoid, Ellipsoids, RefFrames, Transforms
...
if __name__ == '__main__':
    ...
    # defined in all pygeodesy3 modules
    t.testNamedDicts()
    t.testNamedTuples()

    # test __doc__ strings in all pygeodesy3 modules
    for n, m in t.pygeodesy3_names2(deprecated=True):
        with open(n, 'rb') as f:
            py = ub2str(f.read())
